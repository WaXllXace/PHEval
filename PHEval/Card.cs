...
namespace PHEval
{
    public class Card
    {
        ...
        public static void SetPrimeRank(char newPrimeRank) {
            var newRankMap = new Dictionary<char, byte>();
            var newRankReverseArray = new char[13];
            var oldValueOfPrimeRank = rankMap[newPrimeRank];
            
            foreach(var rank in rankMap.Keys) {
                newRankMap[rank] = (byte) ((rankMap[rank] + 12 - oldValueOfPrimeRank) % 13);
            }
            
            for(var i = 0; i < 13; i++) {
                newRankReverseArray[newRankMap[rankReverseArray[i]]] = rankReverseArray[i];
            }
            
            rankMap = newRankMap;
            rankReverseArray = newRankReverseArray;
        }
    }
}